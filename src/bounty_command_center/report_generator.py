...
def generate_run_report(platform: str, stats: dict, run_id: str) -> Path | None:
    """
    Generates a summary report for a harvester run.

    Args:
        platform: The name of the platform that was harvested.
        stats: A dictionary containing run statistics.
        run_id: The unique ID for the run.

    Returns:
        The path to the generated report file, or None if generation failed.
    """
    log.info("Generating run report for platform", platform=platform, stats=stats, run_id=run_id)

    export_dir = Path("reports")
    export_dir.mkdir(parents=True, exist_ok=True)

    timestamp_str = datetime.now().strftime('%Y%m%d-%H%M%S')
    report_path = export_dir / f"run_report_{platform}_{timestamp_str}.md"
...
"""
    ...
    content = f"""
# Harvester Run Report: {platform.title()}

- **Run ID:** `{run_id}`
- **Run Timestamp:** {datetime.now().isoformat()}
- **Run Duration:** {stats.get('duration', 'N/A')}
...
| Metric          | Value |
|-----------------|-------|
| New Programs    | {stats.get('new', 0)}      |
| Invalid Records | {stats.get('invalid', 0)}  |
| Updated Programs| {stats.get('updated', 0)}  |
...
"""
    ...
    try:
        with open(report_path, 'w', encoding='utf-8') as f:
            f.write(content)
        log.info("Successfully generated run report", path=str(report_path))
        return report_path
    except IOError as e:
        log.exception("Failed to write run report to file", path=str(report_path), error=str(e))
        return None
