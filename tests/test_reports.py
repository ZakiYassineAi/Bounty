...
def client_fixture(session: Session):
    ...
    def get_current_user_override():
        ...
        return User(
            id=1, username="testuser", role="admin", password="fakepassword"
        )  # nosec B106
...
def test_generate_pdf_report_for_evidence_unit(session: Session):
    ...
    # 1. Create mock data
    target = Target(
        name="Test Target", url="http://test.com", scope=["http://test.com"]
    )
    session.add(target)
    session.commit()
    ...
    # 3. Assert the result
    assert pdf_bytes is not None  # nosec B101
    assert pdf_bytes.startswith(b"%PDF-")  # nosec B101
...
def test_export_evidence_report_integration(client: TestClient, session: Session):
    ...
    assert response.headers["content-type"] == "application/pdf"  # nosec B101
    assert (  # nosec B101
        response.headers["content-disposition"]
        == f"attachment; filename=evidence_report_{evidence.id}.pdf"
    )
    assert response.content.startswith(b"%PDF-")  # nosec B101
